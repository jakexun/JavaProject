...
public class Red_Write_data {
    ...
    public void Write(String file,String[][] new_data){ //将新产生的数据写入文件
        ...
        for (i = 0;i < new_data.length;i++){
            if (new_data[i][0].equals("null")||new_data[i][1].equals("null")){
    ...
    public static void Statictical(String origin_file,int row,int column,String statictical_file,String result_file) {
        try {
            ...
            for (int i = 0; i < databf.length; i++) {
                if (databf[i][41].equals("normal.")&&databf[i][43]==null) {
                    p += 1;
                } else if (!databf[i][41].equals("normal.")&&databf[i][43]==null) {
                    n += 1;
                }
            }
            int p_n = p + n;
            ...
            for (int i = 0; i < datatd.length; i++) {
                if (!datatd[i][43].equals("{mixture}")&&!(datatd[i][42].equals("null")&&datatd[i][41].equals("normal."))) {
                    tn += 1;
                }
                if (datatd[i][43].equals("{mixture}")){
                    nd+=1;
                }
                if ((datatd[i][43].equals("null")&&datatd[i][41].equals("normal."))){
                    fn+=1;
                }
            }
            int tp = p - fn;
            ...
            String line = "<"+String.format("%1$-7s",p_n)+String.format("%1$-7s",p)+String.format("%1$-7s",n)+">"+"<"+String.format("%1$-9s",datatd.length)
